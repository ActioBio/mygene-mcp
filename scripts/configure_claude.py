#!/usr/bin/env python3
"""Configure Claude Desktop to use MyGene MCP server."""
...
def find_python_executable():
    ...
    if "envs" in python_path and "mygene-mcp" in python_path:
        return python_path
    ...
    try:
        ...
        # Parse output to find mygene-mcp environment
        for line in result.stdout.split('\n'):
            if 'mygene-mcp' in line and not line.startswith('#'):
                parts = line.split()
                if len(parts) >= 2:
    ...
    print("Error: Could not find Python in mygene-mcp conda environment.", file=sys.stderr)
    print("Please activate the mygene-mcp environment and run this script again:", file=sys.stderr)
    print("  conda activate mygene-mcp", file=sys.stderr)
...
def test_server():
    ...
    try:
        ...
        result = subprocess.run(
            [python_path, "-c", "import mygene_mcp.server"],
            capture_output=True,
            text=True,
            cwd=PROJECT_DIR
        )
        if result.returncode != 0:
            print(f"Error: Failed to import mygene_mcp.server", file=sys.stderr)
...
def main():
    ...
    """
    print("--- MyGene MCP Server Configuration for Claude Desktop ---")
